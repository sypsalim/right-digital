...
const DEFAULT_PRICING = {
  ...
  paperGsmRates: {
    '350': { '70x33': 6.00, '50x33': 5.00, 'A3': 4.50, 'A4': 2.50, 'backSide': 2.50 },
    '300': { '70x33': 5.50, '50x33': 4.50, 'A3': 4.00, 'A4': 2.00, 'backSide': 2.00 },
    '250': { '70x33': 5.00, '50x33': 4.50, 'A3': 3.50, 'A4': 2.50, 'backSide': 2.00 },
    '150': { '70x33': 4.00, '50x33': 3.50, 'A3': 3.00, 'A4': 2.00, 'backSide': 1.50 },
    '100': { '70x33': 3.00, '50x33': 2.50, 'A3': 2.50, 'A4': 2.00, 'backSide': 1.50 }
  },
  ...
  stickerRates: {
    'paper': {
      'cymk': { '70x33': 12.00, '50x33': 9.00, 'A3': 6.00, 'A4': 4.00 },
      'cymk_white': { '70x33': 16.00, '50x33': 12.00, 'A3': 8.00, 'A4': 5.00 },
      'clear_color': { '70x33': 14.00, '50x33': 10.50, 'A3': 7.00, 'A4': 4.50 }
    },
    'pvc_white': {
      'cymk': { '70x33': 24.00, '50x33': 18.00, 'A3': 12.00, 'A4': 8.00 },
      'cymk_white': { '70x33': 28.00, '50x33': 21.00, 'A3': 14.00, 'A4': 10.00 },
      'clear_color': { '70x33': 26.00, '50x33': 19.50, 'A3': 13.00, 'A4': 9.00 }
    },
    'pvc_transparent': {
      'cymk': { '70x33': 30.00, '50x33': 22.50, 'A3': 15.00, 'A4': 10.00 },
      'cymk_white': { '70x33': 30.00, '50x33': 22.50, 'A3': 15.00, 'A4': 10.00 },
      'clear_color': { '70x33': 30.00, '50x33': 22.50, 'A3': 15.00, 'A4': 10.00 }
    }
  },
  ...
  laminationRates: {
    ...
    '50x33': 0.25,
    'A3': 0.20,
    'A4': 0.15 // Default added for completeness
  },
  ...
  plotterRates: {
    ...
    '50x33': 3.00,
    'A3': 2.50,
    'A4': 2.00 // Default added for completeness
  },
      ...
      if (currentPricing.paperGsmRates) {
        Object.keys(DEFAULT_PRICING.paperGsmRates).forEach(gsm => {
          if (currentPricing.paperGsmRates[gsm] && currentPricing.paperGsmRates[gsm].backSide === undefined) {
            currentPricing.paperGsmRates[gsm].backSide = DEFAULT_PRICING.paperGsmRates[gsm].backSide;
          }
        });
      }
...
function populateSettingsDrawer() {
  ...
  paperBody.innerHTML = '';
  const gsms = ['350', '300', '250', '150', '100'];
  const sizes = ['70x33', '50x33', 'A3', 'A4', 'backSide'];
  
  gsms.forEach(gsm => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td><strong>${gsm} GSM</strong></td>`;
    sizes.forEach(sz => {
      const val = currentPricing.paperGsmRates[gsm][sz] || 0;
      tr.innerHTML += `
        <td>
  ...
  mats.forEach(mat => {
    colors.forEach(col => {
      const rate70_33 = currentPricing.stickerRates[mat.id][col.id]['70x33'] || 0;
      const rate50_33 = currentPricing.stickerRates[mat.id][col.id]['50x33'] || 0;
      const rateA3 = currentPricing.stickerRates[mat.id][col.id]['A3'] || 0;
      const rateA4 = currentPricing.stickerRates[mat.id][col.id]['A4'] || 0;
      const tr = document.createElement('tr');
      tr.innerHTML = `
        ...
        <td>
          <input type="number" step="0.1" class="table-input sticker-input" 
                 data-mat="${mat.id}" data-col="${col.id}" data-size="70x33" value="${rate70_33.toFixed(2)}">
        </td>
        <td>
          <input type="number" step="0.1" class="table-input sticker-input" 
                 data-mat="${mat.id}" data-col="${col.id}" data-size="50x33" value="${rate50_33.toFixed(2)}">
        </td>
        <td>
          <input type="number" step="0.1" class="table-input sticker-input" 
                 data-mat="${mat.id}" data-col="${col.id}" data-size="A3" value="${rateA3.toFixed(2)}">
        </td>
        <td>
          <input type="number" step="0.1" class="table-input sticker-input" 
                 data-mat="${mat.id}" data-col="${col.id}" data-size="A4" value="${rateA4.toFixed(2)}">
        </td>
      `;
