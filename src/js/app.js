...
const DEFAULT_PRICING = {
  ...
  paperGsmRates: {
    ...
    '250': { '100x70': 1.00, '90x64': 1.00, '50x70': 0.55, '70x33': 1.20, '50x33': 1.00, 'A3': 3.50, 'A4': 2.50, 'backSide': 2.00 },
    '150': { '100x70': 0.80, '90x64': 0.75, '50x70': 0.55, '70x33': 1.00, '50x33': 0.90, 'A3': 3.00, 'A4': 2.00, 'backSide': 1.50 },
  ...
  function updateSection3Labels() {
    ...
    if (currentPaperMode === 'bags') {
      ...
      if (bagFlapInput && (!bagFlapInput.value || bagFlapInput.value === '2.0' || bagFlapInput.value === '1.5')) {
        ...
        const isSmallBag = (bagH <= 30 && bagL <= 30);
        bagFlapInput.value = isSmallBag ? '1.5' : '2.0';
      }
    } else {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div style="display: flex; align-items: center; gap: 4px; justify-content: center;">
            <input type="number" step="1" class="table-input profit-tier-input" data-tier="${item.key}" value="${item.val}">
...
function saveSettingsFromDrawer() {
  ...
  const profitInputs = document.querySelectorAll('.profit-tier-input');
  if (profitInputs.length > 0) {
    if (!currentPricing.profitTiers) currentPricing.profitTiers = {};
    profitInputs.forEach(input => {
      const key = input.getAttribute('data-tier');
      currentPricing.profitTiers[key] = parseFloat(input.value) || 0;
    });
  }
...
function calculateAndUpdate() {
  ...
  if (isBagsMode) {
    ...
    partBagW = sideFold + bagL + bagW; // 1.5 cm Joint + L + W
    partBagH = fullBagH;

    // Check if full bag fits on sheet in either orientation (landscape or portrait):
    const fullFitsLandscape = (fullBagW <= S_W && fullBagH <= S_H);
    const fullFitsPortrait = (fullBagW <= S_H && fullBagH <= S_W);
    const isFullBagFits = fullFitsLandscape || fullFitsPortrait;
        ...
        if (!partFits) {
          boxFoldNotice.innerHTML = `
            <div style="font-weight: 700; margin-bottom: 4px; color: #f43f5e;">
              ⚠️ Notice: Bag Part size (${partBagW.toFixed(1)} × ${partBagH.toFixed(1)} cm) exceeds Sheet Dimensions (${S_W} × ${S_H} cm) even when split into 2 parts!
            </div>
            <div style="font-size: 0.82rem; opacity: 0.9;">
              Full Bag Open Size: ${fullBagW.toFixed(1)} × ${fullBagH.toFixed(1)} cm | Part Size: ${partBagW.toFixed(1)} × ${partBagH.toFixed(1)} cm<br>
              <em>Tip: Try selecting a larger Paper Sheet (e.g. 100×70 cm).</em>
            </div>
          `;
        } else {
          boxFoldNotice.innerHTML = `
            <div style="font-weight: 700; margin-bottom: 4px; color: #eab308;">
              ⚠️ Oversized Bag Notice: Full Bag Size (${fullBagW.toFixed(1)} × ${fullBagH.toFixed(1)} cm) exceeds Sheet Dimensions (${S_W} × ${S_H} cm)!
            </div>
            <div style="font-size: 0.82rem; opacity: 0.9;">
              Bag requires <strong>2 PARTS (2 Sheets per bag)</strong> for printing.<br>
              Each Part Size: <strong>${partBagW.toFixed(1)} × ${partBagH.toFixed(1)} cm</strong> (Joint 1.5 cm + Length ${i_w} cm + Gusset ${i_h} cm | Top Flap ${topFoldVal.toFixed(1)} cm + Height ${boxH} cm + Bottom Flap ${bottomFoldVal.toFixed(1)} cm)<br>
              <em>Total parts required for ${qty} bags = ${qty * 2} parts.</em>
            </div>
          `;
        }
      } else {
  ...
  const itemsToCalculate = (isBagsMode && isTwoPartBag) ? (qty * 2) : qty;
  const baseSheetsNeeded = ups > 0 ? Math.ceil(itemsToCalculate / ups) : 0;
  ...
  // 4. Render Stats
  if (isBagsMode && isTwoPartBag) {
    document.getElementById('valUps').innerText = ups > 0 ? `${ups} Part${ups > 1 ? 's' : ''}/sheet` : '0';
  } else {
    document.getElementById('valUps').innerText = ups;
  }

  if (extraSheets > 0) {
    const partNote = (isBagsMode && isTwoPartBag) ? ` (${qty * 2} parts: 2 parts/bag)` : '';
    document.getElementById('valSheetsNeeded').innerHTML = `${sheetsNeeded} <span style="font-size: 0.72rem; color: var(--primary-gold-light); display: block;">(${baseSheetsNeeded}${partNote} + ${extraSheets} extra)</span>`;
  } else {
    if (isBagsMode && isTwoPartBag) {
      document.getElementById('valSheetsNeeded').innerHTML = `${sheetsNeeded} <span style="font-size: 0.72rem; color: var(--primary-gold-light); display: block;">(${baseSheetsNeeded} sheets: 2 parts/bag)</span>`;
    } else {
  ...
  const profitPercent = getProfitPercent(qty);
  const profitVal = rawTotalCost * (profitPercent / 100);
  const totalCost = rawTotalCost + profitVal;
  ...
  if (isBagsMode) {
    if (isTwoPartBag) {
      document.getElementById('invCutSize').innerText = `${partBagW.toFixed(1)} x ${partBagH.toFixed(1)} cm (2 Parts / Bag)`;
      document.getElementById('invQty').innerText = `${qty} bags (${qty * 2} parts)`;
    } else {
      document.getElementById('invCutSize').innerText = `${fullBagW.toFixed(1)} x ${fullBagH.toFixed(1)} cm (Bag Open Size)`;
      document.getElementById('invQty').innerText = `${qty} bags`;
    }
  } else if (sideFold > 0 || designSpace > 0) {
  ...
  // Profit Margin Row
  const profitRow = document.getElementById('invRowProfit');
  if (profitRow) {
    if (profitPercent > 0 && totalCost > 0) {
      profitRow.classList.remove('hidden');
      const invProfitPercentEl = document.getElementById('invProfitPercent');
      const invProfitValEl = document.getElementById('invProfitVal');
      if (invProfitPercentEl) invProfitPercentEl.innerText = `${profitPercent}%`;
      if (invProfitValEl) invProfitValEl.innerText = `+${profitVal.toFixed(2)} SR`;
    } else {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (currentPaperMode === 'bags') {
        ...
        const isTwoPartBag = (fullBagW > S_W || fullBagH > S_H) && (fullBagW > S_H || fullBagH > S_W);

        const glueFlapPx = sideFold * scale;
        const lengthPx = bagL * scale;
        const gussetPx = bagW * scale;
        const topFoldPx = topFold * scale;
        const heightPx = bagH * scale;
        const bottomFoldPx = bottomFold * scale;
        ...
        if (isTwoPartBag) {
          // --- 2-PART BAG (1 PART DIELINE FORMAT FROM BAG OPEN SIZE 1 PART LOOKS.svg) ---
          const x0 = upX;
          ...
          polyGussetFold.setAttribute("stroke", "#10b981");
          polyGussetFold.setAttribute("stroke-width", "1.2");
          polyGussetFold.setAttribute("stroke-dasharray", "3 2");
          group.appendChild(polyGussetFold);

          // Yellow dimension line marks (matching prompt drawing)
          if (topFoldPx > 3) {
            const xTF = xGlue + (lengthPx * 0.25);
            dimGroup.appendChild(drawYellowArrow(xTF, y0 + 1, xTF, yTop - 1, `TOP FOLD ${topFold.toFixed(1)} CM`, true, -9, 'normal'));
          }
          ...
          if (heightPx > 8) {
            const xH = xGlue + (lengthPx * 0.5);
            dimGroup.appendChild(drawYellowArrow(xH, yTop + 2, xH, yBodyEnd - 2, `HEIGHT ${bagH} CM`, true, -9, 'normal'));
          }

          if (lengthPx > 10) {
            const yL = yTop + (heightPx * 0.25);
            dimGroup.appendChild(drawYellowArrow(xGlue + 3, yL, xL1 - 3, yL, `LENGTH ${bagL} CM`, false, -6, 'normal'));
          }

          if (gussetPx > 10) {
            const yG = yTop + (heightPx * 0.25);
            dimGroup.appendChild(drawYellowArrow(xL1 + 3, yG, xG1End - 3, yG, `GUSSET ${bagW} CM`, false, -6, 'normal'));
          }

          if (bottomFoldPx > 5) {
            const xF = xGlue + (lengthPx * 0.5);
          ...
          text.textContent = (cols * rows > 1) ? `Part ${upCounter}` : `Part 1 / 2`;
          group.appendChild(text);

        } else {
          // --- SINGLE SHEET FULL BAG (BAG OPEN SIZE LOOKS.svg FORMAT) ---
          const x0 = upX;
          const xGlue = upX + glueFlapPx;
          const xL1 = xGlue + lengthPx;
          const xG1Mid = xL1 + halfGussetPx;
...
function calculateTshirtCupAndUpdate() {
  ...
  let discountVal = 0;
  let baseTotalAfterDisc = totalCost;
  let discountPercent = 0;

  if (qty > 10) {
    discountPercent = 10;
    discountVal = totalCost * 0.10;
    baseTotalAfterDisc = totalCost - discountVal;
  }
  ...
  const profitPercent = getProfitPercent(qty);
  const profitVal = baseTotalAfterDisc * (profitPercent / 100);
  const finalTotal = baseTotalAfterDisc + profitVal;
  ...
  const profitRow = document.getElementById('invRowProfit');
  if (profitRow) {
    if (profitPercent > 0 && finalTotal > 0) {
      profitRow.classList.remove('hidden');
      const invProfitPercentEl = document.getElementById('invProfitPercent');
      const invProfitValEl = document.getElementById('invProfitVal');
      if (invProfitPercentEl) invProfitPercentEl.innerText = `${profitPercent}%`;
